...
// Timer que reseta a cada pergunta
function SessionTimer({ questionIndex }) {
  ...
  return (
    <span style={timerText}>⏱ {mins}:{secs}</span>
  );
}
...
export default function SessionLivePage() {
  ...
  // Busca nome do quiz e total de perguntas
  useEffect(() => {
  ...
  // Escuta jogadores em tempo real
  useEffect(() => {
    const q = query(
  ...
  // Conta respondidos da pergunta atual
  useEffect(() => {
    if (!session) return;
  ...
  // Redireciona para a turma quando sessão encerrar
  useEffect(() => {
    ...
    <div style={container}>
      ...
      <div style={topBar}>
        <div>
          ...
          <p style={codigoLabel}>
            Código: <strong>{session.pin}</strong>
          </p>
        </div>
...
const quizLabel = {
  fontSize: "18px", fontWeight: "bold", color: "#222", margin: "0 0 4px"
};
const codigoLabel = {
...
const timerText = {
  fontSize: "22px", fontWeight: "bold", color: "#4CAF50"
};
const progressText = {
